...
void LCD_Menu(){
  ...
  if(settingMode==1){
    ...
    //BUTTON KEYPRESS
    if(setMenuPage==0){
      if(digitalRead(buttonRight)==0){subMenuPage++;}
      if(digitalRead(buttonLeft)==0) {subMenuPage--;}
      if(digitalRead(buttonBack)==0) {settingMode=0;subMenuPage=0;}  //bool engage, main menu int page
      if(digitalRead(buttonSelect)==0){setMenuPage=1;} //enter sub menu settings - bool engage 
      lcdBacklight_Wake();
      while(digitalRead(buttonRight)==0||digitalRead(buttonLeft)==0    
      ||digitalRead(buttonBack)==0||digitalRead(buttonSelect)==0){}
    } 
    ...
    if(subMenuPage==0){
      ...
      if(setMenuPage==0){boolTemp = MPPT_Mode;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}MPPT_Mode = boolTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}
      }     
    }
    ...
    ///// SETTINGS MENU ITEM: CHARER/PSU MODE /////
      ...
      if(setMenuPage==0){boolTemp = output_Mode;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}output_Mode = boolTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}
      }     
    }
    ...
    ///// SETTINGS MENU ITEM: MAX BATTERY V /////
      ...
      if(setMenuPage==0){floatTemp = voltageBatteryMax;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}voltageBatteryMax = floatTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        currentMenuSetMillis = millis();
        if(digitalRead(buttonRight)==0){                                                    //Right button press (increments setting values)
          while(digitalRead(buttonRight)==0){
        ...
        else if(digitalRead(buttonLeft)==0){                                                //Left button press (decrements setting values)
          while(digitalRead(buttonLeft)==0){
            if(millis()-currentMenuSetMillis>longPressTime){                                //Long Press     
              voltageBatteryMax -= 1.00;                                                    //Increment by 1
              voltageBatteryMax = constrain(voltageBatteryMax,vOutSystemMin,vOutSystemMax); //Limit settings values to a range
              lcd.setCursor(2,1);lcd.print(voltageBatteryMax,2);delay(longPressInterval);   //Display settings data                               
    ...
    ///// SETTINGS MENU ITEM: MIN BATTERY V /////
      ...
      if(setMenuPage==0){floatTemp = voltageBatteryMin;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}voltageBatteryMin = floatTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        currentMenuSetMillis = millis();
        if(digitalRead(buttonRight)==0){                                                    //Right button press (increments setting values)
          while(digitalRead(buttonRight)==0){
    ...
    ///// SETTINGS MENU ITEM: CHARGING CURRENT /////
      ...
      if(setMenuPage==0){floatTemp = currentCharging;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}currentCharging = floatTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        currentMenuSetMillis = millis();
        if(digitalRead(buttonRight)==0){                                                  //Right button press (increments setting values)
          while(digitalRead(buttonRight)==0){
    ...
    ///// SETTINGS MENU ITEM: COOLING FAN /////
      ...
      if(setMenuPage==0){boolTemp = enableFan;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}enableFan = boolTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}
      } 
    }
    ///// SETTINGS MENU ITEM: FAN TRIG TEMP /////
      ...
      if(setMenuPage==0){intTemp = temperatureFan;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}temperatureFan = intTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        if(digitalRead(buttonRight)==0){                                              //Right button press (increments setting values)
          while(digitalRead(buttonRight)==0){   
    ...
    ///// SETTINGS MENU ITEM: SHUTDOWN TEMP /////
      ...
      if(setMenuPage==0){intTemp = temperatureMax;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}temperatureMax = intTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        if(digitalRead(buttonRight)==0){                                              //Right button press (increments setting values)
          while(digitalRead(buttonRight)==0){   
    ...
    ///// SETTINGS MENU ITEM: WIFI FEATURE /////
      ...
      if(setMenuPage==0){boolTemp = enableWiFi;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}enableWiFi = boolTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}
      }       
    }
    ...
    ///// SETTINGS MENU ITEM: AUTOLOAD /////
      ...
      if(setMenuPage==0){boolTemp = flashMemLoad;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}flashMemLoad = boolTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveAutoloadSettings();setMenuPage=0;savedMessageLCD();}
      }       
    }
    ///// SETTINGS MENU ITEM: BACKLIGHT SLEEP /////
      ...
      if(setMenuPage==0){intTemp = backlightSleepMode;}
        ...
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}backlightSleepMode = intTemp;cancelledMessageLCD();setMenuPage=0;} 
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}saveSettings();setMenuPage=0;savedMessageLCD();}     
        if(digitalRead(buttonRight)==0){                                                    //Right button press (increments setting values)
    ...
    ///// SETTINGS MENU ITEM: FACTORY RESET /////
      ...
      if(setMenuPage==0){
        ...
        if(digitalRead(buttonRight)==0||digitalRead(buttonLeft)==0){while(digitalRead(buttonRight)==0||digitalRead(buttonLeft)==0){}if(confirmationMenu==0){confirmationMenu=1;}else{confirmationMenu=0;}}  //Cycle Yes NO
        if(digitalRead(buttonBack)==0){while(digitalRead(buttonBack)==0){}cancelledMessageLCD();setMenuPage=0;confirmationMenu=0;} //Cancel
        if(digitalRead(buttonSelect)==0){while(digitalRead(buttonSelect)==0){}if(confirmationMenu==1){factoryReset();factoryResetMessageLCD();}setMenuPage=0;confirmationMenu=0;subMenuPage=0;}
      } 
    }  
    ///// SETTINGS MENU ITEM: FIRMWARE VERSION /////
      ...
      if(setMenuPage==0){
        ...
        if(digitalRead(buttonBack)==0||digitalRead(buttonSelect)==0){while(digitalRead(buttonBack)==0||digitalRead(buttonSelect)==0){}setMenuPage=0;} //Cancel
      } 
    }  
  }
  //MAIN MENU
    ...
    //BUTTON KEYPRESS
    if(digitalRead(buttonRight)==0) {buttonRightCommand=1;lcdBacklight_Wake();}
    if(digitalRead(buttonLeft)==0)  {buttonLeftCommand=1;lcdBacklight_Wake();}
    if(digitalRead(buttonBack)==0)  {buttonBackCommand=1;lcdBacklight_Wake();}
    if(digitalRead(buttonSelect)==0){buttonSelectCommand=1;lcdBacklight_Wake();}
